...
program PsHMain
	...
	interface
		integer(c_int) function optimizewavefn(MaxIter, Omega, NumTerms, LValue, M12max, M23max, M31max, pmax, IsTriplet, Ordering, Method, EigenRoutine, alpha, beta, gamma) bind(c, name='optimizewavefn')
		use iso_c_binding
		integer(c_int), value :: MaxIter, Omega, NumTerms, LValue, M12max, M23max, M31max, pmax, IsTriplet, Ordering, Method, EigenRoutine
...
!    deallocate(WMatrix)
	...
	if (Optimize == 1) then
		Alpha8 = Alpha;  Beta8 = Beta;  Gamma8 = Gamma;
		write (*,*) "Beginning optimization of nonlinear parameters"
		fa = optimizewavefn(MaxIter, Omega, NumTerms, LValue, M12max, M23max, M31max, pmax, IsTriplet, Ordering, Method, EigenRoutine, Alpha8, Beta8, Gamma8)
		!fa = EDeriv(Omega, NumTerms, Alpha, Beta, Gamma, EigenNum, PhiPhi, Phi2HPhi, M12max, M23max, M31max, pmax, &
	...
	call WriteData(iwrite, LValue, NumTerms, PhiPhi, Phi2HPhi)
...
subroutine WriteData(iwrite, LValue, NumTerms, PhiPhi, Phi2HPhi)
	implicit none
	integer iwrite, LValue, NumTerms, NumTermsUsed, i, j
	real*16, dimension(NumTerms,NumTerms) :: PhiPhi, Phi2HPhi

	NumTermsUsed = NumTerms
    if (LValue == 0) then  ! S-wave only has one symmetry
        NumTermsUsed = NumTerms / 2
	end if

	write (iwrite,'(a)') "<data>"
	do i = 1, NumTermsUsed, 1
		!do j = 1, i, 1  ! Use this instead if only the lower triangle is required.
		do j = 1, NumTermsUsed, 1
...
subroutine CalcEigenvalues(iwrite, LValue, NumTerms, AllEnergies, PhiPhi, Phi2HPhi, LowerEigen, UpperEigen)
	implicit none
	integer iwrite, LValue, NumTerms, NumTermsUsed, AllEnergies, StartEnergy, LowerEigen, UpperEigen
	real*16, dimension(NumTerms,NumTerms) :: PhiPhi, Phi2HPhi
	real*8, allocatable, dimension(:,:) :: PhiPhi8, Phi2HPhi8
	real*8, allocatable, dimension(:) :: Energies, Workspace
	integer i, j, Info
	...
	if (AllEnergies == 0) then
		StartEnergy = NumTermsUsed
	else
		StartEnergy = 1
	end if

	if (UpperEigen > NumTermsUsed) UpperEigen = NumTermsUsed

	do j = StartEnergy, NumTermsUsed, 1
	!do j = 1, 1, 1
		! Copies our real*16 matrices to real*8 matrices so that LAPACK can operate on them.
		Phi2HPhi8 = Phi2HPhi
		PhiPhi8 = PhiPhi
...
!@TODO: Precalculate C coefficients?
...
subroutine CalcMatrices(RunCalc, UsePreCalc, PowerTabler1i, PowerTabler1j, PowerTabler2i, PowerTabler2j, WMatrix, PhiPhi, Phi2HPhi, NumTerms, &
    ...
    if (LValue == 0) return  ! S-wave only has one symmetry
...
subroutine ederivwrapper(Omega, NumTerms, LValue, M12max, M23max, M31max, pmax, IsTriplet, Ordering, Method, EigenRoutine, Alpha, Beta, Gamma, Iter, Energy) bind(C)
	implicit none
	real*16, allocatable, dimension(:,:) :: PhiPhi, Phi2HPhi
	real*8, allocatable, dimension(:,:) :: PhiPhi8, Phi2HPhi8
	real*8, allocatable, dimension(:) :: Energies, Workspace
	integer Omega, NumTerms, LValue, M12max, M23max, M31max, pmax, IsTriplet, Ordering, Method, EigenNum, EigenRoutine, Info, Iter, NumUsed
	real*8 Alpha, Beta, Gamma, Energy
	real*16 Alpha16, Beta16, Gamma16, EDeriv
	...
	Energies = 0.0q0
	EigenNum = 1  !@TODO: Change to allow other values

	Alpha16 = Alpha;  Beta16 = Beta;  Gamma16 = Gamma;
	
	Energy = EDeriv(Omega, NumTerms, Alpha16, Beta16, Gamma16, EigenNum, PhiPhi, Phi2HPhi, &
						M12max, M23max, M31max, pmax, IsTriplet, Ordering, Method, EigenRoutine, LValue)
	...
	!if (EigenRoutine == 1) then
		...
		PhiPhi8 = PhiPhi
		Phi2HPhi8 = Phi2HPhi
		if (LValue == 0) then  ! S-Wave
			NumUsed = NumTerms / 2
		else
			NumUsed = NumTerms
		end if
		call dsygv(1, 'N', 'L', NumUsed, Phi2HPhi8, NumTerms, PhiPhi8, NumTerms, Energies, Workspace, 3*NumTerms-1, Info)
		! TODO: Test Info!
		write (*,*) "Info: ", Info
		deallocate(PhiPhi8, Phi2HPhi8)
	!else  ! EigenRoutine == 2
	!	call Newdsygv(1, 'N', 'L', NumUsed, Phi2HPhi, NumTerms, PhiPhi, NumTerms, Energies, Workspace, 3*NumTerms-1, Info)
	!end if
	...
	write (10,*)
	write (0,"(I4,A,3F8.5)") Iter, "; Alpha, Beta, Gamma: ", Alpha, Beta, Gamma  ! I dislike hardcoding the 10, but it's easier at this point.
	write (0,"(A,F16.12)") "Energy: ", Energy
	write (0,*)
	flush(10)
...
real*16 function EDeriv(Omega, NumTerms, Alpha, Beta, Gamma, EigenNum, PhiPhi, Phi2HPhi, &
	...
	integer Omega, NumTerms, M12max, M23max, M31max, pmax, IsTriplet, Method, Ordering, i, j, n, TempJ, Info, EigenRoutine, LValue
	real*16, dimension(NumTerms,NumTerms) :: PhiPhi, Phi2HPhi
	real*16, allocatable, dimension(:,:,:,:) :: WMatrix
...
!@TODO: Change to a subroutine
300	EDeriv = 1.0d0  ! Random value, return not used in this case.
