...
real*8 function FactorialGeneral(n)
	implicit none
	integer n
	integer i
	...
	if (n < 0) then
		FactorialGeneral = 0.0_8
		!write (*,*) 'n < 0 in factorial function'
		return
	endif

	FactorialGeneral = 1.0_8
	do i = 1, n
		FactorialGeneral = FactorialGeneral * real(i,8)
	enddo
...
real*8 function DoubleFactorial(n)
	implicit none
	integer n
	integer i
	real*8 Fact
...
real*8 function BinomialGeneral(n, k)
	implicit none
	integer n, k
	...
	if (k > n) then
		BinomialGeneral = 0.0d0
		return
	end if
	
	BinomialGeneral = FactorialGeneral(n) / (FactorialGeneral(k) * FactorialGeneral(n-k))
	return
end


! S is given in equation () - just a simple minimum function.
real*8 function SGeneral(q, j)
	implicit none
	integer q, j
	SGeneral = min(real(q-1,8), 0.5_8 * real(j+1,8))
	return
end


! C is given in equation (4).
real*8 function CCoeff(j, q, k)
	implicit none
	integer j, q, k
	real*8 SGeneral, BinomialGeneral, DoubleFactorial
	real*8 LeadingFactor
	real*8 Multiplication
	real*8 SVal
	integer t

	! C for the case of j12 == -2 is given by equation (129).
	if (j == -2) then
		CCoeff = real(2*q + 1,8) * DoubleFactorial(2*q + 2*k) * DoubleFactorial(2*k - 1) &
			/ (DoubleFactorial(2*q + 2*k + 1) * DoubleFactorial(2*k))
		return
	endif

	LeadingFactor = real(2*q + 1,8) / real(j + 2,8) * BinomialGeneral(j+2, 2*k + 1)
	Multiplication = 1.0d0
	SVal = SGeneral(q, j)

	do t = 0, int(SVal), 1
		Multiplication = Multiplication * real(2*k + 2*t - j,8) / real(2*k + 2*q - 2*t + 1,8)
	enddo
...
real*8 function WGeneral(l, m, n, alpha, beta, gamma, pmax)
	implicit none
	integer l, m, n
	real*8 alpha, beta, gamma
	integer pmax
	real*8 FactorialGeneral
	real*8 WPartialGeneral
	real*8 LeadingFactor
	real*8 Summation
	integer p

	LeadingFactor = FactorialGeneral(l) / ((alpha + beta + gamma) ** (l+m+n+3))
	Summation = 0.0
...
real*8 function WPartialGeneral(l, m, n, alpha, beta, gamma, p)
	implicit none
	integer l, m, n
	real*8 alpha, beta, gamma
	integer p
	real*8 FactorialGeneral
	real*8 Hypergeometric2F1General

	WPartialGeneral = FactorialGeneral(l+m+n+p+2) / (FactorialGeneral(l+1+p) * (l+m+2+p)) * (alpha / (alpha + beta + gamma)) ** p  * &
		Hypergeometric2F1General(1.0_8, real(l+m+n+p+3, 8), real(l+m+p+3, 8), real((alpha + beta) / (alpha + beta + gamma), 8))
...
real*8 function Hypergeometric2F1General(a, b, c, z)
	implicit none
	real*8 a, b, c, z
	!integer a, b, c
	!real*8 z
	real*8 Sum, Term, Prev, nd
	integer n, nUpper

	Sum = 1.0_8
	Prev = 0.0_8
	nUpper = 1000  ! Only for extremely singular 2F1 will it even reach close to this number.
	!nUpper = 65

	Term = 1.0_8
	do n = 1, nUpper, 1
		nd = n
		Term = Term * (a+nd-1.0_8) * (b+nd-1.0_8) / ((c+nd-1.0_8) * nd) * z
		Sum = Sum + Term
	...
	Hypergeometric2F1General = Sum
end function Hypergeometric2F1General


! TODO: Parameter order
real*8 function IAngular(l1p, m1p, l2p, m2p, l3p, m3p, l1, m1, l2, m2, l3, m3, q12, q23, q31, SphHarm, sl, sr)
	implicit none
	integer l1p, m1p, l2p, m2p, l3p, m3p, l1, m1, l2, m2, l3, m3, q12, q23, q31, SphHarm, sl, sr, bfunc, b
	integer Triangular
	real*8 ThreeJSymbol, SixJSymbol, CP12Angular, CP21Angular, CP23Angular, CP31Angular
	real*8 Ang, Term
	integer T1, T2, T3, T1min, T1max, T2min, T2max, T3min, T3max
	integer n1, n2, n3, n1min, n1max, n2min, n2max, n3min, n3max
	real*8 c, Removec1, Removec2
		...
		case (3,6)  ! This is the case where we are calculating equation 59 for C^P(r2.r3)
			...
			do T2 = T2min, T2max, 1
				do T3 = T3min, T3max, 1
					...
					Ang = Ang + b * CP23Angular(l1p, m1p, l2p, m2p, l3p, m3p, l1, m1, l2, m2, l3, m3, q12, q23, q31, T2, T3)
				end do
			end do
			...
			do n1 = n1min, n1max, 1
				do n2 = n2min, n2max, 1
					do n3 = n3min, n3max, 1
						...
						Ang = Ang + Term
						!if (Term == 0.0_8) exit  ! Every term past here should be 0.  @TODO: Verify this.
					enddo
				enddo
			enddo

			Ang = Ang * (-1.0_8)**(m1p+m2p+m3p+q12+q23+q31) * &
				dsqrt(real((2*l1p+1)*(2*l2p+1)*(2*l3p+1)*(2*l1+1)*(2*l2+1)*(2*l3+1),8))
	end select
...
real*8 function CP12Angular(l1p, m1p, l2p, m2p, l3p, m3p, l1, m1, l2, m2, l3, m3, q12, q23, q31, T1, T2)
	implicit none
	integer Triangular, l1p, m1p, l2p, m2p, l3p, m3p, l1, m1, l2, m2, l3, m3, q12, q23, q31
	integer T1, T2, T1min, T1max, T2min, T2max, bfunc
	integer n1, n2, n3, n1min, n1max, n2min, n2max, n3min, n3max
	real*8 ThreeJSymbol, SixJSymbol, CP12AngularTilde
	real*8 U, ThreeJs, CP, Mult
	...
	U = dsqrt(dble((2*l1p+1)*(2*l2p+1)*(2*l3p+1)*(2*l1+1)*(2*l2+1)*(2*l3+1))) * (-1.0d0)**(q12+q23+q31)
		...
		do n2 = n2min, n2max, 1
			do n3 = n3min, n3max, 1
				...
				ThreeJs = ThreeJSymbol(1, l1, T1, 0, 0, 0) * ThreeJSymbol(1, l2, T2, 0, 0, 0) &
					...
					* ThreeJSymbol(l3p, l3, n3, 0, 0, 0) * ThreeJSymbol(q31, q12, n1, 0, 0, 0) &
					* ThreeJSymbol(q12, q23, n2, 0, 0, 0) * ThreeJSymbol(q23, q31, n3, 0, 0, 0)
				if (ThreeJS == 0.0d0) then  ! No need to calculate the rest of the term, since it will be 0.
					continue
				end if
...
real*8 function CP12AngularTilde(l1p, m1p, l2p, m2p, l3p, m3p, l1, m1, l2, m2, l3, m3, n1, n2, n3, T1, T2)
	implicit none
	integer l1p, m1p, l2p, m2p, l3p, m3p, l1, m1, l2, m2, l3, m3, q12, q23, q31, T1, T2, n1, n2, n3, mu
	real*8 ThreeJSymbol

	CP12AngularTilde = 0.0d0
	do mu = -1, 1, 1
		CP12AngularTilde = CP12AngularTilde + (-1.0d0)**(-m3+mu) * ThreeJSymbol(1, l1, T1, mu, m1, -mu-m1) * ThreeJSymbol(1, l2, T2, -mu, m2, mu-m2) &
...
real*8 function CP23Angular(l1p, m1p, l2p, m2p, l3p, m3p, l1, m1, l2, m2, l3, m3, q12, q23, q31, T2, T3)
	implicit none
	integer Triangular, l1p, m1p, l2p, m2p, l3p, m3p, l1, m1, l2, m2, l3, m3, q12, q23, q31
	integer T2, T3, T1min, T1max, T2min, T2max, bfunc
	integer n1, n2, n3, n1min, n1max, n2min, n2max, n3min, n3max
	real*8 ThreeJSymbol, SixJSymbol, CP23AngularTilde
	real*8 U, ThreeJs, CP, Mult
...
real*8 function CP23AngularTilde(l1p, m1p, l2p, m2p, l3p, m3p, l1, m1, l2, m2, l3, m3, n1, n2, n3, T2, T3)
	implicit none
	integer l1p, m1p, l2p, m2p, l3p, m3p, l1, m1, l2, m2, l3, m3, q12, q23, q31, T2, T3, n1, n2, n3, mu
	real*8 ThreeJSymbol

	CP23AngularTilde = 0.0d0
	do mu = -1, 1, 1
		CP23AngularTilde = CP23AngularTilde + (-1.0d0)**(-m1+mu) * ThreeJSymbol(1, l2, T2, mu, m2, -mu-m2) * ThreeJSymbol(1, l3, T3, -mu, m3, mu-m3) &
...
real*8 function CP31Angular(l1p, m1p, l2p, m2p, l3p, m3p, l1, m1, l2, m2, l3, m3, q12, q23, q31, T3, T1)
	implicit none
	integer Triangular, l1p, m1p, l2p, m2p, l3p, m3p, l1, m1, l2, m2, l3, m3, q12, q23, q31
	integer T3, T1, T1min, T1max, T2min, T2max, bfunc
	integer n1, n2, n3, n1min, n1max, n2min, n2max, n3min, n3max
	real*8 ThreeJSymbol, SixJSymbol, CP31AngularTilde
	real*8 U, ThreeJs, CP, Mult
...
real*8 function CP31AngularTilde(l1p, m1p, l2p, m2p, l3p, m3p, l1, m1, l2, m2, l3, m3, n1, n2, n3, T3, T1)
	implicit none
	integer l1p, m1p, l2p, m2p, l3p, m3p, l1, m1, l2, m2, l3, m3, q12, q23, q31, T3, T1, n1, n2, n3, mu
	real*8 ThreeJSymbol

	CP31AngularTilde = 0.0d0
	do mu = -1, 1, 1
		CP31AngularTilde = CP31AngularTilde + (-1.0d0)**(-m2+mu) * ThreeJSymbol(1, l1, T1, -mu, m1, mu-m1) * ThreeJSymbol(1, l3, T3, mu, m3, -mu-m3) &
...
real*8 function WR(RunCalc, UsePreCalc, q12, q23, q31, k12, k23, k31, j1, j2, j3, j12, j23, j31, alpha, beta, gamma, pmax, WMatrix)
	use WLimits
	implicit none
...
! TODO: Parameter order
real*8 function HylleraasIntegralGeneral(RunCalc, UsePreCalc, j1, j2, j3, j12, j23, j31, alpha, beta, gamma, &
								l1p, l2p, l3p, m1p, m2p, m3p, l1, l2, l3, m1, m2, m3, qmax, pmax, linf, minf, SphHarm, sl, sr, WMatrix, Method)
								!l1p, m1p, l2p, m2p, l3p, m3p, l1, m1, l2, m2, l3, m3, qmax)
	use WLimits
	implicit none
	real*8, dimension(lmin:lmax, mmin:mmax, nmin:nmax, 6) :: WMatrix
	integer j1, j2, j3, j12, j23, j31, l1p, m1p, l2p, m2p, l3p, m3p, l1, m1, l2, m2, l3, m3, qmax, pmax, linf, minf, sl, sr
	logical RunCalc, UsePreCalc
	real*8, allocatable, dimension(:) :: Values
	real*8 Radial, Angular, Constants, Asymp, alpha, beta, gamma, AsymptoticPart
	real*8 IAngular, WR, CCoeff, InnerSum, OuterSum, RemoveMe
	integer Mj, Lij, SphHarm, l, i, Method
 ...
 !
	...
	do q12 = 0, M12, 1
		...
		!if (Values(q12) == 0.0d0) then
		!	exit
		!endif

		OuterSum = OuterSum + InnerSum
		Radial = 0.0d0  ! TODO: These three lines needed?
		Angular = 0.0d0
		Constants = 0.0d0
	enddo

	!write (*,'(d24.16)') 1984.4017075391884912_8*OuterSum
	...
	if (Method == 0 .or. q12 < minf) then
		HylleraasIntegralGeneral = 1984.4017075391884912_8*OuterSum
		return
	end if

	! @TODO: Skipping the asymptotic part right now
	!write (*,*) 'Doing asymptotic part'
!	Asymp = 1984.4017075391884912_8*OuterSum
!	!do l = 0, L12, 1
!	do l = 1, L12, 1
!		if (l <= 2) then
!			RemoveMe = AsymptoticPart(Values, 2.0d0, l, 0, 5000)
!		else
!			RemoveMe = AsymptoticPart(Values, 2.0d0, l, min(l-2,15), 5000)  ! TODO: Is 15 good enough? Maybe put as a parameter.
!		endif
!
!		OuterSum = 0.0d0
!		do i = 0, l-1, 1
...
!
!!		write (*,*) l, "Sum:", 1984.4017075391884912_8 * OuterSum, " Asymptotic:", 1984.4017075391884912_8 * Asymp
!	enddo
	...
	enddo
	Asymp = OuterSum !+ RemoveMe
	Asymp = Asymp * 1984.4017075391884912_8

	deallocate(Values)

	HylleraasIntegralGeneral = Asymp !OuterSum
	return
end
