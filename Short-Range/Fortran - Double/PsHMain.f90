...
program PsHMain
	...
	real*8 Tol, Err, h, EDeriv, fa, fh, y, Divisor
	...
	if (Optimize == 1) then
		...
		fa = optimizewavefn(MaxIter, Omega, NumTerms, LValue, M12max, M23max, M31max, pmax, IsTriplet, Ordering, Method, EigenRoutine, Alpha, Beta, Gamma)
		goto 200
	else
		...
		fa = EDeriv(Omega, NumTerms, Alpha, Beta, Gamma, EigenNum, PhiPhi, Phi2HPhi, PhiPhi8, Phi2HPhi8, M12max, M23max, M31max, pmax, &
...
subroutine ederivwrapper(Omega, NumTerms, M12max, M23max, M31max, pmax, IsTriplet, Ordering, Method, EigenRoutine, Alpha, Beta, Gamma, LValue, Iter, Energy) bind(C)
	implicit none
	real*16, allocatable, dimension(:,:) :: PhiPhi, Phi2HPhi
	real*8, allocatable, dimension(:,:) :: PhiPhi8, Phi2HPhi8
	real*8, allocatable, dimension(:) :: Energies, Workspace
	integer Omega, LValue, NumTerms, M12max, M23max, M31max, pmax, IsTriplet, Ordering, Method, EigenRoutine, Info, Iter, NumUsed
